...
/** The own-entrance kinds whose entrance IS workspace_commit, through the server's surface build
 * gate (activatable-kinds.ts `COMMIT_GATED_KINDS`) — /cynap-push commits them. */
export const COMMIT_GATED_KINDS = new Set(['surface-source']);
...
export function entranceForKind(kind) {
  if (COMMIT_GATED_KINDS.has(kind)) return null;
...
/** The SDK bundle owns each kind's effect; this mirrors the server's commit-level pre-check. */
export function checkPushEffects(paths, classifyPath, effectForKind) {
  const effects = new Set();
  const armed = [];
  const handlerId = (path) =>
    /^automations\/handlers\/([^/]+)\/(?:handler\.ts|config\.json)$/.exec(path)?.[1]
    ?? /^automations\/([^/]+)\.ts$/.exec(path)?.[1]
    ?? null;
  for (const path of paths) {
    const kind = classifyPath(path);
    if (kind === 'generated' || kind === 'unknown') continue;
    const effect = effectForKind(kind);
    if (effect === 'schema') effects.add('schema');
    if (effect === 'handler') effects.add(`handler:${handlerId(path) ?? path}`);
    if (effect === 'post_deploy') armed.push({ path, kind });
  }
  const [singleEffect] = effects;
  const incompatible = singleEffect !== undefined && armed.some(({ path, kind }) =>
    !(singleEffect.startsWith('handler:') && kind === 'handler-config' && handlerId(path) === singleEffect.slice('handler:'.length))
  );
  return effects.size > 1 || incompatible ? [...effects].sort() : null;
}
