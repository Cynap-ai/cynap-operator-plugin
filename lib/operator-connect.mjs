...
import { PROXY_VERSION } from '../bin/operator-proxy.mjs';
...
export async function runOperatorConnect({
  ...
  if (connectPlan.action === 'reuse' || connectPlan.action === 'wait') {
    ...
    if (connectPlan.health?.version !== PROXY_VERSION) {
      throw new Error(
        `operator connect: existing connector is not the supported proxy version ${PROXY_VERSION}; disconnect it and retry`
      );
    }
    const health =
