...
function connectedResult(plan, health, reused) {
  return {
    status: 'connected',
    reused,
    slug: plan.slug,
...
export async function runOperatorConnect({
  ...
  if (!pluginVersion) {
    throw new Error('operator connect: pluginVersion is required (CYN-1959 — read fresh from plugin.json by the caller)');
  }
  ...
  if (connectPlan.action === 'reuse' || connectPlan.action === 'wait') {
    ...
    if (connectPlan.health?.pluginVersion !== pluginVersion) {
      throw new Error(
        `operator connect: existing connector is running plugin version ${connectPlan.health?.pluginVersion ?? 'unknown'}, ` +
          `not the current plugin version ${pluginVersion}; disconnect it and retry`
      );
    }
    const health =
