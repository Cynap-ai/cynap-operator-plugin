...
async function requestProxyDisconnect({ slug, fetchImpl = fetch }) {
  const port = stablePortForSlug(slug);
  const controlPath = join(resolveWorkingDir(slug), CONTROL_FILE);
  const controlNonce = readFileSync(controlPath, 'utf8').trim();
  if (!controlNonce) throw new Error('operator disconnect: local control nonce is missing');
