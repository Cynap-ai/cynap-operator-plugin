#!/usr/bin/env node
// Testable mechanics behind /cynap-connect <org>.
//
...
//
// Login is MODE-keyed, not env-keyed: the bundled proxy's default is
// PKCE-loopback (opens a browser, yields a CLI-scoped `octk_…` credential —
// see operator-proxy.mjs pkceLoopbackLogin), `--device` is the
...
// mode: cynap-e2e on staging stays headless (`--e2e`, the previous
...
// the platform service re-resolves org access
...
import { CYNAP_E2E_ORG_ID, HEALTH_PATH } from '../bin/operator-proxy.mjs';

export { CYNAP_E2E_ORG_ID };
 ...
 * (operator-proxy.mjs DEFAULT_ORG_SLUG / DEFAULT_TARGET_ORG_ID).
 ...
 * class this import exists to prevent: two disagreeing string literals with
 ...
 * --allow-org pin. The AUTHORITATIVE org resolution is server-side: the
 ...
 * platform tooling for no functional gain (the platform is a generic
...
/**
 * Pick the proxy login mode for a (slug, env) pair:
 *   - cynap-e2e on staging → 'e2e' (the headless e2e-session cookie leg — keeps
 *     the agent-driven dogfood flow browser-free, matching previous behavior).
...
export function materializeWorkingDir({ slug, mcpJson }) {
  ...
  const port = Number(new URL(mcpJson.mcpServers['cynap-operator'].url).port);
  const codexConfigPath = writeCodexMcpConfig({ dir, toml: buildCodexMcpToml({ port }) });
  return { dir, mcpJsonPath, codexConfigPath };
}
 ...
 *
 * `headers.X-Cynap-CC-Session` carries `${CLAUDE_SESSION_ID}` — a
 ...
 * Code re-parses `.mcp.json` on later-session connects, so a later session
...
/** Codex's project-scoped loopback MCP declaration. The proxy, not this file,
 * owns the short-lived bearer credential. */
export function buildCodexMcpToml({ port, serverName = 'cynap-operator' }) {
  if (!Number.isInteger(port) || port <= 0) {
    throw new Error(`buildCodexMcpToml: invalid port ${port}`);
  }
  if (!/^[A-Za-z0-9_-]+$/.test(serverName)) {
    throw new Error('buildCodexMcpToml: invalid server name');
  }
  return `[mcp_servers.${serverName}]\nurl = "http://127.0.0.1:${port}/mcp"\n`;
}

/** Atomic temp-plus-rename write: Codex sees either a complete old config or
 * a complete new config, never an interrupted declaration. */
export function writeCodexMcpConfig({ dir, toml }) {
  const codexDir = join(dir, '.codex');
  mkdirSync(codexDir, { recursive: true });
  const path = join(codexDir, 'config.toml');
  const temporary = join(codexDir, `.config.toml.${process.pid}.${Date.now()}.tmp`);
  writeFileSync(temporary, toml, { mode: 0o600 });
 ...
 * operator-proxy.mjs parseArgs) — this is the client-side
 * pin; the server-side check in operator-token/route.ts is authoritative.
 * `authMode` selects the login leg: 'e2e' / 'device' append the
 * matching flag; 'interactive' (PKCE loopback) is the proxy's default and
 * appends nothing. `--org-slug` is the session-trail marker
...
export async function planConnect({
  ...
  // token, and the proxy adopts it post-login. Those modes therefore never pin
  // `--allow-org`, even for a slug the map knows: cynap-e2e over PKCE must take
  // the same server-resolved path as any customer org, which is what the
  // release journey's connect leg proves. An unknown slug is NOT an error there
  // either: it is simply an org whose id only the server is entitled to
  ...
  // (the platform stays a generic engine, no per-customer code).
  const orgId = authMode === 'e2e' ? resolveOrgId(slug) : null;
  if (!orgId && authMode === 'e2e') {
    throw new Error(
  ...
  const { dir, mcpJsonPath, codexConfigPath } = materialize({ slug, mcpJson });
