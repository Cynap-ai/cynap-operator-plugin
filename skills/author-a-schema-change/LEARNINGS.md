...
- **Naming the silent-skip gap explicitly** (gotcha 3) rather than implying
  ...
  covers the blocked-keyword and punctuation patterns; other per-entity
  failures still get
...
- Adding a new field to an entity that already had a `dedicated_table`,
  ...
  table is created once, on first registration; the column
  only appears via a separate path that is not fully hardened. Always
...
- `dedicated_table` is a one-way door in practice: declare the full field
  ...
  requiring explicit verification, not routine work.
...
- Should this skill document the `unique_index` field's interaction with
  `dedicated_table` (does it become a real uniqueness constraint, or is
  it advisory)? Current: not verified in
